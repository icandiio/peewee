...
class Expr(Leaf):
    def __init__(self, lhs, op, rhs, negated=False):
        ...
        self.op = op  # 运算符
        self.rhs = rhs  # 运算符右侧
        self.negated = negated  # ~:非运算

    def clone(self):
        return Expr(self.lhs, self.op, self.rhs, self.negated)


class DQ(Leaf):  # DotQuery => query 点运算 => 递归
...
class Func(Leaf):
    ...
    def __getattr__(self, attr):  # 实现递归
        def dec(*args, **kwargs):
...
class Field(Leaf):
    ...
    def add_to_class(self, model_class, name):  # model __new__ 解析 Field 构造 model 实例
        self.name = name
        ...
        setattr(model_class, name, FieldDescriptor(self))  # @important
...
class QueryCompiler(object):  # sql,params 解析生成
    field_map = {
        'int': 'INTEGER',
        'bigint': 'INTEGER',
        'float': 'REAL',
    ...
    def __init__(self, model, cursor, meta=None):
        ...
        if self.naive:
            ...
            for i in range(len(self.cursor.description)):
                col = self.cursor.description[i][0]  # cursor.description = > ((col1,...),(col2,...)) # 结果集字段描述信息【二维数据】
...
class Query(object):
    ...
    def _execute(self):  # @important，语句执行体
        sql, params = self.sql()  # 构造sql语句 + 参数列表
        return self.database.execute_sql(sql, params, self.require_commit)  # sql执行

    def execute(self):  # 执行入口，内部调用_execute()
        raise NotImplementedError
...
class ModelOptions(object):  # @important => Model._meta = ModelOptions() => Model数据模型和操作的内部数据结构
...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        # 收集继承的三大类数据 => attrs:字段信息,_meta:表信息,_data：数据信息
        # _meta 后期会汇集所有的表信息（字段，主键，索引等）
        ...
        for name, attr in cls.__dict__.items():
            ...
            if isinstance(attr, Field):
                attr.add_to_class(cls, name)  # @important 拆解数据，构造目标Class的辅助信息（_meta字段汇集所有信息）
                if attr.primary_key:
                    primary_key = attr
...
class Model(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():  # 通过 FieldDescriptor 写入到 _data 里面
            setattr(self, k, v)
